...
interface MeterFinalInspectionTableProps {
  displayItems: any[];
...
export default function MeterFinalInspectionTable({
  displayItems,
  selections,
  onSelectGrade,
  onOpenDetail,
  ...
  onToggleSelectAll,
}: MeterFinalInspectionTableProps) {
  const selectableIds = React.useMemo(() => {
    return displayItems
      .filter((it) => !it.isTotalRow && !it.isStartRow && !it.is_deleted && it.status_inspeksi !== "Dihapus" && it.status_mending !== "Dihapus" && it.status_final_mending !== "Dihapus")
      .map((it) => it.id);
  }, [displayItems]);
    ...
    <table className="w-full min-w-[720px] text-left text-xs border-collapse">
      <thead>
        <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-extrabold text-slate-500 uppercase tracking-wider">
          ...
          <th className="px-2 py-2 min-w-[160px] w-full text-center border-r border-slate-100" rowSpan={2}>KETERANGAN CACAT & MENDING</th>
          <th className="px-2 py-2 text-center w-24 border-r border-slate-100" rowSpan={2}>AKSI</th>
          <th className="px-2 py-1 border-b border-slate-200 font-extrabold text-slate-600 text-center border-r border-slate-100" colSpan={3}>FINAL INSPEK</th>
        </tr>
      ...
      <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
        {displayItems.map((item: any, index: number) => {
          ...
          return (
            ...
            >
              {onToggleSelectDetail && (
                <td className="px-1.5 py-1.5 text-center border-r border-slate-100 border-b border-slate-100">
                  {!isDeleted ? (
                    <input
                      type="checkbox"
              ...
              <td className="sticky left-0 z-10 bg-white px-1 py-1.5 font-bold text-slate-800 text-center text-xs w-7 border-r border-slate-100 border-b border-slate-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                <div className="flex flex-col items-center">
                  ...
                  {isDeleted && (
                    <span className="text-[8px] font-black bg-rose-100 text-rose-700 px-1 py-0.2 rounded mt-0.5 leading-none">
                      DIHAPUS
                    </span>
                  )}
                </div>
              ...
              <td className="px-3 py-1.5 text-[11px] font-medium whitespace-pre-line leading-tight border-r border-slate-100 border-b border-slate-100">
                {(() => {
                  const renderCacatContent = () => {
                  ...
                  return isDeleted ? (
                    <div className="italic text-slate-400 font-medium">[Baris Dihapus]</div>
                  ) : (item.isIstirahat || item.hasIstirahat) ? (
                    <>
                  ...
                  <div className="flex items-center justify-center gap-1">
                    {onOpenAddQC && (
                      <button
                        ...
                        className="p-1 rounded bg-white border border-slate-200 text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition-all cursor-pointer"
                    ...
                    {onOpenEditDetail && (
                      <button
                        ...
                        className="p-1 rounded bg-white border border-slate-200 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 transition-all cursor-pointer"
                        title="Koreksi Data"
                      >
                    ...
                    <button
                      onClick={() => onDeleteDetail({ id: item.id, panelNo: item.displayNo, name: item.cacatDisplay || "Detail Meter" })}
                      className="p-1 rounded bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer"
                      title="Hapus Rincian"
                    >
              ...
              <td className="px-1 py-1.5 text-center border-r border-slate-100 border-b border-slate-100 w-16">
                {isDeleted ? (
                  <span className="text-slate-300 font-bold block text-center">-</span>
                ) : item.isGradable ? (
                  <button
                    onClick={() => onSelectGrade(item.id, "A")}
                    className={`w-7 h-7 mx-auto flex items-center justify-center rounded-md transition-all border ${
                      selections[item.id] === "A"
              ...
              <td className="px-1 py-1.5 text-center border-b border-slate-100 w-16">
                {isDeleted ? (
                  <span className="text-slate-300 font-bold block text-center">-</span>
                ) : item.isGradable ? (
                  <button
                    ...
                    className={`w-7 h-7 mx-auto flex items-center justify-center rounded-md transition-all border ${
                      selections[item.id] === "BS"
                        ? "border-rose-500 bg-rose-100 text-rose-700 shadow-sm"
                        : "border-slate-200 bg-white text-slate-300 hover:border-rose-300 hover:text-rose-500"
                    }`}
                  >
