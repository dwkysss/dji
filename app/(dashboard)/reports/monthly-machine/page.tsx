          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <p className="text-xs sm:text-sm font-medium text-slate-500 mt-2">
                Laporan rekapitulasi produksi dan cacat per bulan untuk masing-masing mesin.
              </p>
            </div>

            {/* Right: Filter & Action Controls sticking to the right */}
            <div className="flex flex-col items-start md:items-end gap-2.5 shrink-0 self-start md:self-auto w-full md:w-auto pt-4 md:pt-0 border-t border-slate-100 md:border-t-0">
              {/* Row 1: Filters */}
              <div className="flex flex-wrap items-center justify-start md:justify-end gap-2.5 w-full md:w-auto">
                {/* Mesin Dropdown */}
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-50/90 hover:bg-slate-100/90 rounded-xl border border-slate-200/80 transition-colors shadow-2xs">
                  <Monitor className="w-4 h-4 text-[#0070bc]" />
                  <select
                    value={selectedMachine}
                    onChange={(e) => setSelectedMachine(e.target.value)}
                    className="bg-transparent text-sm font-black text-slate-700 outline-none cursor-pointer min-w-[70px]"
                  >
                    {machines.map((m) => (
                ...
                {/* Periode Dropdown */}
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-50/90 hover:bg-slate-100/90 rounded-xl border border-slate-200/80 transition-colors shadow-2xs">
                  <Calendar className="w-4 h-4 text-[#0070bc]" />
                  <select
                    value={selectedMonth}
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="h-10 w-10 bg-[#0070bc] hover:bg-[#005a96] active:scale-95 text-white rounded-xl shadow-md shadow-[#0070bc]/20 transition-all flex items-center justify-center cursor-pointer shrink-0 disabled:opacity-50"
                  title="Refresh / Muat Ulang Data Terbaru"
                >
                  <RotateCw className={`w-4 h-4 ${isLoading || isBackgroundUpdating ? "animate-spin" : ""}`} />
                </button>

                {/* Auto-Sync Schedule Button */}
                <button
                  type="button"
                  onClick={() => setIsScheduleModalOpen(true)}
                  className={`h-10 px-3 rounded-xl border text-xs font-bold transition-all flex items-center gap-2 cursor-pointer shadow-2xs ${
                    scheduleEnabled
                      ? "bg-amber-50/80 border-amber-300 text-amber-900 hover:bg-amber-100"
                      : "bg-slate-50/90 border-slate-200/80 text-slate-500 hover:bg-slate-100"
                  }`}
                  title="Klik untuk mengatur jam sinkronisasi otomatis harian"
                >
                  <div className="relative">
                    <Clock className={`w-3.5 h-3.5 ${scheduleEnabled ? "text-amber-600" : "text-slate-400"}`} />
                    {scheduleEnabled && (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px] text-amber-700 font-extrabold">WIB</span>
                  </div>
              ...
              <div className="flex flex-wrap items-center justify-start md:justify-end gap-2.5 w-full md:w-auto">
                <button
                  ...
                  className="h-10 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-extrabold text-xs shadow-md shadow-indigo-600/20 transition-all flex items-center gap-2 disabled:opacity-50 cursor-pointer whitespace-nowrap"
                ...
                <button
                  ...
                  onClick={() => {
                    setSyncAllResults(null);
                    setSyncTargetMachines(
                      machines.length > 0
                        ? [...machines]
                        : ["R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8", "R9", "R10", "R11", "T1C", "T2A"]
                    );
                    setIsSyncAllModalOpen(true);
                  }}
                  disabled={isLoading}
                  className="h-10 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 active:scale-95 text-white font-extrabold text-xs shadow-md shadow-orange-500/20 transition-all flex items-center gap-2 disabled:opacity-50 cursor-pointer whitespace-nowrap"
                  title="Sync Mesin Terpilih / Seluruh Mesin Sekaligus"
                >
                  <Zap className="w-4 h-4 fill-current" />
                  <span>Sync Semua Mesin</span>
                </button>
                ...
                <button
                  ...
                  disabled={isLoading || reportData.length === 0}
                  className="h-10 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-extrabold text-xs shadow-md shadow-emerald-600/20 transition-all flex items-center gap-2 disabled:opacity-50 cursor-pointer whitespace-nowrap"
