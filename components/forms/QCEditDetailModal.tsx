...
interface QCEditDetailModalProps {
  ...
  onSuccess: (detailId: string | string[], newGrade: number, updatedData?: any) => void;
}
...
export default function QCEditDetailModal({
  ...
  const isBulk = !!(selectedDetailIds && selectedDetailIds.length > 1 && selectedDetailIds.includes(detail?.id));
  ...
  const handleSave = async () => {
    ...
    try {
      const defectObjects = selectedQCDefects.map((name) => {
        const existing = detail.production_defects?.find((d: any) => {
          const cleanExisting = String(d.detail || "").replace(/^\[QC\]\s*/i, "").trim();
          return cleanExisting.toLowerCase() === name.toLowerCase();
        });
        return {
          kategori: existing?.kategori || "A",
          detail: name,
          blok: existing?.blok || undefined,
          meter: existing?.meter || undefined,
        };
      });
      ...
      if (mode === "add_qc") {
        const isBulkAction = !!(selectedDetailIds && selectedDetailIds.length > 1 && selectedDetailIds.includes(detail.id));
        const targetIds = isBulkAction ? selectedDetailIds : [detail.id];
        ...
        if (res.success) {
          onSuccess(targetIds.length > 1 ? targetIds : detail.id, 3, res.updatedData);
          onClose();
        } else {
